...
void ImageWrite(IMAGE *image,char *filename)
{
	...
	if(r!=0) fwrite(image->palette,4*r,1,fp);//if the image has color palette the palette is written to the file
...
void setZero(IMAGE *image)
{
	...
	for(i=0;i<image->bmpih.bih*rowsize;i++) image->data[i]=50;
}

int main()
{
	IMAGE *image=(IMAGE*)malloc(sizeof(IMAGE));
	image=ImageRead(image,"test2.bmp");
	//writeInfo(image,"biber2.bmp");
	setZero(image);
	ImageWrite(image,"test4.bmp");
