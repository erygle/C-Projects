...
void lowFilter(IMAGE *image,double filter[3][3]){
	int h,w,rowsize,i;
	h=image->bmpih.bih;
	w=image->bmpih.biw;
	BYTE mask[3][3];
	...
	for(i=1 ; i<h-1 ; i++){
		for(int j=1;j<rowsize-1;j++){
			for(int k=-1 ; k<2 ; k++){
				for(int l=-1 ; l<2 ; l++){
					mask[k+1][l+1]=image->data[(i+k)*rowsize+j+l];
				}
			}
			sum = 0;
			for(int k=0 ; k<3 ; k++){
...
int main()
{
	...
	double filter[3][3] = {1.9,1.9,1.9,1.9,1.9,1.9,1.9,1.9,1.9,};
	image=ImageRead(image,"kelebek.bmp");
	writeInfo(image,"kelebek.bmp");
    //negativeImage(image);
    //changeColorPalette(image);
    //thresholdImage(image,100);
    //thresholdImage(image);
	//lowFilter(image,filter);
    setZero(image);
	ImageWrite(image,"masking.bmp");
	free(image);   
